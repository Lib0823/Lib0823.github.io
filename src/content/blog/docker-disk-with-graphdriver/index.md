...
- 디스크 마운트 없이 컨테이너를 실행하면, 컨테이너 내에서 생성되는 모든 데이터는 docker 호스트의 파일 시스템에 저장됩니다.

구체적으로, 데이터는 Docker 호스트 시스템의 그래프 드라이버에 의해 관리되는 특정 디렉터리(예: overlay2, aufs, btrfs 등)에 저장됩니다.

1. 그래프 드라이버란?

그래프 드라이버는 Docker의 파일 시스템 레이어를 관리하는 방식을 정의하는 도구입니다. 여러 종류의 그래프 드라이버가 있으며, 각 드라이버는 Docker가 파일 시스템 레이어를 어떻게 처리할지를 결정합니다.

파일 시스템 레이어는 여러 계층으로 구성된 파일 시스템 구조로 컨테이너가 실행될 때 여러 레이어를 결합하여 최종 파일 시스템을 제공한다.

2. 주요 그래프 드라이버 종류

Docker는 다양한 파일 시스템 드라이버를 지원하며, 각 드라이버는 레이어를 다루는 방식이 다릅니다. 주로 사용되는 그래프 드라이버는 다음과 같습니다:
...
**overlay2**는 Docker의 오버레이 파일 시스템을 기반으로 한 드라이버로, Linux에서 매우 효율적인 파일 시스템을 제공합니다. overlay2는 Copy-on-Write(COW) 방식을 사용하여 효율적으로 파일 시스템을 관리합니다.

### Overlay2의 작동 방식

overlay2는 읽기 전용 이미지(base image)와 쓰기 가능한 레이어(container layers)로 구성된 계층적 구조로 작동합니다. 컨테이너를 생성할 때마다 새로운 레이어가 추가됩니다. 이 구조는 크게 LowerDir, UpperDir, MergedDir, WorkDir로 나눌 수 있습니다.

3-1. LowerDir (읽기 전용 레이어)

기본 이미지의 파일이 위치한 곳입니다.

여러 개의 읽기 전용 레이어로 구성되며, 일반적으로 Docker 이미지가 생성될 때 이곳에 저장됩니다.

LowerDir에 저장된 파일들은 다른 컨테이너가 읽을 수 있으며, 읽기 전용이기 때문에 변경할 수 없습니다.

3-2. UpperDir (쓰기 가능한 레이어)

컨테이너에서 파일을 변경하거나 새로 생성한 내용은 UpperDir에 저장됩니다.

각 컨테이너마다 독립적인 UpperDir가 존재하며, 여기서 변경된 내용은 컨테이너가 종료되기 전까지 지속됩니다.

예를 들어, 컨테이너 내에서 파일을 수정하거나 생성할 때, 그 변경 사항은 UpperDir에 저장됩니다.

3-3. MergedDir (병합된 디렉터리)

**LowerDir**의 읽기 전용 파일과 **UpperDir**의 쓰기 가능한 파일이 병합된 파일 시스템을 제공합니다.

실제로 컨테이너가 파일 시스템을 액세스할 때 사용되는 디렉터리입니다. 사용자는 여기에서 파일을 보고, 수정할 수 있습니다.

3-4. WorkDir (작업 디렉터리)

파일 시스템 병합을 위해 사용하는 임시 작업 디렉터리입니다.

파일 시스템의 변경 작업, 즉 Copy-on-Write 작업을 수행할 때 필요한 공간입니다.

실제로 컨테이너의 운영 중에 사용되며, 사용자에게는 직접적으로 노출되지 않습니다.
