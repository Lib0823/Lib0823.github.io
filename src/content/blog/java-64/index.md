---
title: '자바란?'
description: '자바는 C언어에 객체 지향적 기능을 추가하여 만든 C++과 달리, 처음부터 객체 지향 언어로 개발된 프로그래밍 언어이다. 자바는 자바 가상 머신(JVM, Java Virtual Machine)을 사용하여...'
date: 2023-08-20
category: 'Language'
tags: ['Java']
draft: false
...
3. 하이브리드 언어이다.
  ...
  - 여기서 더 나아가 JVM(정확히는 HotSpot)은 반복적으로 실행되는(hot) 코드를 감지해서 그 부분만 네이티브 머신 코드로 다시 컴파일하는 JIT(Just-In-Time) 컴파일을 수행한다. 그래서 초반엔 인터프리터처럼 느리다가도, 실행이 길어질수록 점점 빨라지는 특성이 있다.
4. 메모리를 자동으로 관리한다.
...
1. 자바는 실행을 위해 자바 가상 머신을 거쳐야 하므로, C/C++처럼 네이티브로 컴파일되는 언어에 비해 실행 속도가 느리다. (다만 JIT 컴파일 덕분에 오래 실행되는 서버 애플리케이션에서는 그 격차가 크지 않다.)
