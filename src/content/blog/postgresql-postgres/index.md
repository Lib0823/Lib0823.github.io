...
다만 지우기 전에 postmaster.pid에 적힌 PID로 실제 프로세스가 살아있는지 먼저 확인해야 한다. 파일만 지우고 새 서버를 띄웠는데 기존 프로세스가 진짜로 살아서 같은 데이터 디렉터리에 접근 중이었다면, 두 프로세스가 동시에 같은 파일을 건드리면서 데이터가 깨질 수 있다.
