...
description: '최근 클라우드 컴퓨팅 환경에서 자주 등장하는 키워드 중 하나가 바로 ''서버리스(Serverless)''입니다. AWS Lambda, Google Cloud Functions 같은 서비스들이 바로 서버리스 컴퓨팅의...'
date: 2026-01-05
...
최근 클라우드 컴퓨팅 환경에서 자주 등장하는 키워드 중 하나가 바로 '서버리스(Serverless)'입니다. AWS Lambda, Google Cloud Functions 같은 서비스들이 바로 서버리스 컴퓨팅의 대표적인 예입니다. 하지만 이름과는 달리, 서버리스는 서버가 없다는 의미가 아닙니다. 이 글에서는 서버리스의 개념부터 실제 구현까지 자세히 알아보겠습니다.
...
**서버리스(Serverless)**는 말 그대로 서버가 없다는 뜻이 아니라, **"사용자가 신경 써야 할 서버 관리 작업이 없다"**는 의미입니다.
...
- 특정 이벤트(웹사이트 버튼 클릭, 이미지 업로드 등)가 발생할 때만 코드가 실행됩니다.
...
서버가 없는데 어떻게 웹사이트를 배포할 수 있을까요? 전통적인 방식은 서버 한 대를 통째로 빌려 사용했지만, 서버리스 환경에서는 역할을 분담합니다.
...
많은 분들이 AWS Lambda가 JavaScript(Node.js)만 지원한다고 생각하지만, 실제로는 거의 모든 주요 프로그래밍 언어를 지원합니다.
...
최근에는 도커 이미지 자체를 Lambda에 올릴 수 있어 언어의 제약이 완전히 사라졌습니다.
...
서버리스 환경에서 실제로 어떻게 요청이 처리되는지 시각적으로 살펴보겠습니다.
...
클라이언트가 Lambda 함수를 호출하려면 어떻게 해야 할까요?

### 7.1 API Gateway 방식 (전통적)

Lambda 자체는 함수일 뿐이라 인터넷 주소(URL)가 없습니다. API Gateway가 Lambda에 URL을 부여하는 역할을 합니다.
...
API Gateway 없이 Lambda 함수 설정에서 버튼 클릭 한 번으로 직접 호출 가능한 URL을 생성할 수 있습니다.
...
두 방식 모두 클라이언트 입장에서는 일반적인 REST API와 동일합니다:
...
서버리스 컴퓨팅은 AWS만의 전유물이 아닙니다. 주요 클라우드 제공업체들이 모두 자체 서버리스 플랫폼을 제공하고 있습니다.
...
**클라우드 없이 자체 인프라에서도 서버리스를 구축할 수 있습니다!**
...
자바 개발자들이 서버리스를 처음 접할 때 가장 당황하는 부분입니다. Spring Framework의 무거운 기능들을 걷어내고 순수 자바 객체(POJO) 형태로 작성하는 것이 기본입니다.
...
전통적인 Spring Boot는 서버리스 환경에서 너무 무겁습니다. 이를 해결하기 위한 경량 프레임워크들:
...
자바의 최대 약점인 Cold Start를 해결하는 기술입니다.
...
거의 Python 수준의 Cold Start 성능을 달성할 수 있습니다.
...
서버리스는 "필요할 때만 잠깐 쓰고, 서버 관리는 하기 싫고, 사용한 만큼만 돈을 내고 싶을 때" 최적의 선택입니다.
...
서버리스는 특히 스타트업, 이벤트성 서비스, 마이크로서비스 아키텍처에 강력한 솔루션입니다. 하지만 24시간 지속적인 트래픽이나 긴 실행 시간이 필요한 작업에는 전통적인 서버 방식이 더 적합할 수 있습니다.
